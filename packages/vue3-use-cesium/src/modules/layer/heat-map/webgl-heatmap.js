...
const textureFloatShims = function () {
	...
	checkSupport = function () {
		let canvas, extobj, gl, halfFloatExt, halfFloatTexturing, singleFloatExt, singleFloatTexturing;
		canvas = document.createElement("canvas");
		gl = null;
		try {
...
const WebGLHeatmap = (function () {
	function WebGLHeatmap(_arg) {
		...
		if (window.WebGLDebugUtils != null) {
			// eslint-disable-next-line @typescript-eslint/no-unused-vars
			this.gl = window.WebGLDebugUtils.makeDebugContext(this.gl, function (err, funcName, args) {
				// eslint-disable-next-line no-undef
				throw window.WebGLDebugUtils.glEnumToString(err) + " was caused by call to: " + funcName;
			});
		}
