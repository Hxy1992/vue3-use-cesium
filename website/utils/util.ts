...
export function getBrowserLang() {
	...
	let defaultBrowserLang: System.languageType = "zh";
 ...
 * 打开路由地址
 * @param path 路由地址
 * @param target 新窗口/当前窗口
 */
export const openPath = (path: string, target: "_self" | "_blank" = "_self") => {
 ...
 * @returns Map
 */
export function uuidToMap(uuids: string | string[]) {
	if (!uuids) return;
	const uuidMap: System.MapType<string, boolean> = {};
...
/**
 * 重写setInterval方法，页面隐藏时暂停
 * @param callback 回调函数
 * @param ms 间隔（毫秒）
 * @param immediate 是否立即执行（默认否）
 * @returns clearInterval
 */
export function setIntervalPref(callback: () => void, ms: number, immediate = false) {
	if (immediate) callback();
	let timer: NodeJS.Timer | undefined = setInterval(callback, ms);
